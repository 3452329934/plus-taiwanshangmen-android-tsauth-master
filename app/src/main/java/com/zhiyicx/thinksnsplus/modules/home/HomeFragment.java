...
public class HomeFragment extends TSFragment<HomeContract.Presenter> implements DynamicFragment.OnCommentClickListener, HomeContract.View,
    ...
    protected void initData() {
        ...
        setCurrentPage();
        // app更新
        if (mPresenter != null && mPresenter.checkAppUpgradeEnable()) {
    ...
    public MainFragment getMainFragment() {
        return ((MainFragment) mFragmentList.get(PAGE_HOME));
    }
...
//                    ((MainFragment) mFragmentList.get(mCurrenPage)).refreshCurrentPage();
    ...
    private void setJpushAlias() {
        if (mPresenter.isLogin()) {
            // 设置极光推送别名
            JpushAlias jpushAlias = new JpushAlias(getContext().getApplicationContext(), "user_" + String.valueOf(AppApplication.getMyUserIdWithdefault()));
            jpushAlias.setAlias();
        }
