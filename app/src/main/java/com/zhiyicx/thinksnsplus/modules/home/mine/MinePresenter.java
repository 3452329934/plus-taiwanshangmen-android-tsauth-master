...
public class MinePresenter extends AppBasePresenter<MineContract.View> implements MineContract.Presenter {
    ...
    public void getUserInfoFromDB() {
        ...
        Subscription subscribe = Observable.<UserInfoBean>create(subscriber -> {
            // 尝试从数据库获取当前用户的信息
            UserInfoBean singleDataFromCache = mUserInfoBeanGreenDao.getSingleDataFromCache(AppApplication.getMyUserIdWithdefault());
            WalletBean walletBean = mWalletBeanGreenDao.getSingleDataFromCacheByUserId(AppApplication.getMyUserIdWithdefault());
            if (walletBean != null) {
                int ratio = mSystemRepository.getBootstrappersInfoFromLocal().getWallet().getRatio();
///                    walletBean.setBalance(walletBean.getBalance() * (ratio / MONEY_UNIT));
                singleDataFromCache.setWallet(walletBean);
            }
            subscriber.onNext(singleDataFromCache);
...
//        addSubscrebe(mNewMessageSub);
    ...
    public void setFollowFansCount(UserFollowerCountBean data) {
        ...
        if (data != null && data.getUser() != null) {
            int commentCount = data.getUser().getCommented();
            int likeCount = data.getUser().getLiked();
            int systemCount = data.getUser().getSystem();
            int pinnedNums;
            pinnedNums = data.getUser().getFeedCommentPinned()
                    + data.getUser().getNewsCommentPinned()
                    + data.getUser().getGroupJoinPinned()
                    + data.getUser().getPostCommentPinned()
                    + data.getUser().getPostPinned();
            TSEMHyphenate.getInstance().setMsgUnreadCount(followingCount
                    + firendsCount + commentCount + likeCount + systemCount + pinnedNums);
        }
                ...
                .subscribe(new BaseSubscribeForV2<UserInfoBean>() {
                    ...
                    protected void onSuccess(UserInfoBean data) {
                        ...
                        if (!TextUtils.isEmpty(userInfoBean.getLocalAvatar())) {
                            data.setAvatar(new Avatar(userInfoBean.getLocalAvatar()));
                        }
