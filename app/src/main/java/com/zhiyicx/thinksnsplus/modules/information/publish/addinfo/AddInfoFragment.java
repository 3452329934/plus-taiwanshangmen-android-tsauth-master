...
import com.zhiyicx.thinksnsplus.modules.information.publish.detail.EditeInfoDetailFragmentV2;
...
public class AddInfoFragment extends TSFragment<AddInfoContract.Presenter> implements AddInfoContract.View {
    ...
    public void onBackPressed() {
        if (EditeInfoDetailFragmentV2.mInfoPublishBean != null) {
            EditeInfoDetailFragmentV2.mInfoPublishBean.setFrom(mTvFrom.getEditInput().getText().toString());
            EditeInfoDetailFragmentV2.mInfoPublishBean.setAuthor(mTvAuthor.getEditInput().getText().toString());
            EditeInfoDetailFragmentV2.mInfoPublishBean.setSubject(mEtInfoSummary.getInputContent());
        }
        mActivity.finish();
    }
    ...
    protected void initView(View rootView) {
        ...
        if (EditeInfoDetailFragmentV2.mInfoPublishBean.getTags() != null) {
            mUserTagBeens.addAll(EditeInfoDetailFragmentV2.mInfoPublishBean.getTags());
        }
        if (!TextUtils.isEmpty(EditeInfoDetailFragmentV2.mInfoPublishBean.getCategoryName())) {
            mToolbarRight.setEnabled(true);
        }
        if (!TextUtils.isEmpty(EditeInfoDetailFragmentV2.mInfoPublishBean.getCategoryName())) {
            mBtAddCategory.setRightText(EditeInfoDetailFragmentV2.mInfoPublishBean.getCategoryName());
        }
        mTvFrom.setEditInputString(EditeInfoDetailFragmentV2.mInfoPublishBean.getFrom());
        mTvAuthor.setEditInputString(EditeInfoDetailFragmentV2.mInfoPublishBean.getAuthor());
        mEtInfoSummary.setText(EditeInfoDetailFragmentV2.mInfoPublishBean.getSubject());
    ...
    private void initListener() {
        ...
        RxTextView.textChanges(mTvFrom.getEditInput())
                .compose(this.bindToLifecycle())
                .subscribe(charSeques -> EditeInfoDetailFragmentV2.mInfoPublishBean.setFrom(charSeques.toString()));
        // 作者
        RxTextView.textChanges(mTvAuthor.getEditInput())
                .compose(this.bindToLifecycle())
                .subscribe(charSeques -> EditeInfoDetailFragmentV2.mInfoPublishBean.setAuthor(charSeques.toString()));
        // 摘要
        RxTextView.textChanges(mEtInfoSummary.getEtContent())
                .compose(this.bindToLifecycle())
                .subscribe(charSeques -> EditeInfoDetailFragmentV2.mInfoPublishBean.setSubject(charSeques.toString()));
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && data != null && data.getExtras() != null) {
            ...
            if (requestCode == TagFrom.INFO_PUBLISH.id) {
                ...
                EditeInfoDetailFragmentV2.mInfoPublishBean.setTags(mUserTagBeens);
                if (mUserTagBeens.size() == 0) {
                    mTagsHint.setVisibility(View.VISIBLE);
                } else {
                    mTagsHint.setVisibility(View.GONE);
                }
                ...
                EditeInfoDetailFragmentV2.mInfoPublishBean.setCategoryId(category.getId());
                EditeInfoDetailFragmentV2.mInfoPublishBean.setCategoryName(category.getName());
    ...
    private void checkNextButton() {
        if (EditeInfoDetailFragmentV2.mInfoPublishBean.getCategoryId() != 0 && !mUserTagBeens.isEmpty()) {
            mToolbarRight.setEnabled(true);
        } else {
            mToolbarRight.setEnabled(false);
        }
