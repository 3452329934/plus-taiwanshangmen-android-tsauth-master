...
public class GalleryPictureFragment extends TSFragment<GalleryConstract.Presenter> implements View.OnLongClickListener, PhotoViewAttacher
    ...
    protected void showInputPsdView(boolean show) {
        if (getArguments() != null && show) {
            boolean animationIn = getArguments().getBoolean("animationIn");
            if (animationIn) {
    ...
    private void loadImage(final ImageBean imageBean, final AnimationRectBean rect) {
        ...
        if (mImageBean.getImgUrl() != null && FileUtils.isFileExists(mImageBean.getImgUrl())) {
            ...
            needAnimation = true;
            local.into(new GallaryGlideDrawableImageViewTarget(rect, needAnimation, imageBean));
            // 聊天里面用
                    ...
                    .load(new CustomImageSizeModelImp(imageBean) {
                        ...
                        public GlideUrl requestGlideUrl() {
                            if (TextUtils.isEmpty(imageBean.getListCacheUrl()) || isNeedOrin) {
                                ...
                                if (!GlideCahceUtil.hasCache(context, imageBean.getListCacheUrl())) {
                                    setGalleryPotoBgColor();
                                    startLoadProgress();
                                }
    ...
    /**
     * 记录当前的加载状态，用于延迟动画的判断。需求时，延时一段时间后再显示加载动画
     */
    ...
    private void startLoadProgress() {
        ...
        isLoading = true;
        mLoadingSub = Observable.timer(1500, TimeUnit.MILLISECONDS)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(aLong -> {
                    if (isLoading && mPbProgressImage != null && mPbProgressImage.getVisibility() == View.GONE) {
    ...
    private void setGalleryPotoBgColor() {
        mFlGalleryPhoto.setBackgroundResource(R.color.black);
    }
    ...
    public void onDestroy() {
        ...
        if (mLoadingSub != null && !mLoadingSub.isUnsubscribed()) {
            mLoadingSub.unsubscribe();
        }
        super.onDestroy();
    }
    ...
    public void onSureClick(View v, String text, InputPasswordView.PayNote payNote) {
        mPresenter.payNote((long) payNote.dynamicPosition, mImageBean.getPosition(), mImageBean.getToll().getPaid_node(), payNote.isImage, payNote.psd);
    }
    ...
    private void initCenterPopWindow(int resId, boolean isSavePic) {
        ...
        mPayPopWindow = PayPopWindow.builder()
                ...
                .buildMoneyStr(String.format(getString(R.string.buy_pay_integration), "" + mImageBean.getToll().getToll_money()))
                .buildCenterPopWindowItem1ClickListener(() -> {
                    if (mPresenter.usePayPassword()) {
                        mIlvPassword.setPayNote(new InputPasswordView.PayNote(mImageBean.getFeed_id().intValue(), mImageBean.getPosition(),
                                mImageBean.getToll().getPaid_node(), isSavePic, null));
                        showInputPsdView(true);
                    } else {
                        mPresenter.payNote(mImageBean.getFeed_id(), mImageBean.getPosition(), mImageBean.getToll().getPaid_node(), isSavePic, null);
                    }
                    mPayPopWindow.hide();
                })
