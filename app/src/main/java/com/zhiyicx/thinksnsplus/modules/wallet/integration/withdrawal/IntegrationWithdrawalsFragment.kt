...
class IntegrationWithdrawalsFragment : TSFragment<IntegrationWithdrawalsContract.Presenter>(), IntegrationWithdrawalsContract.View {
    ...
    override fun initData() {
        ...
        mEtInput.hint = getString(R.string.et_input_withdrawals_integration_tip_format, mSystemConfigBean.currency.settings.cashmin, mGoldName)
        initListener()
    ...
    private fun setDynamicRatio(currentIntegration: Int) {
        mTvMineIntegration.text = getString(R.string.integration_2_money_ratio_formart, currentIntegration, mGoldName, PayConfig.realCurrencyFen2Yuan((currentIntegration.toFloat() /mSystemConfigBean.currency.settings.rechargeratio).toDouble()))
    }
    ...
    private fun initListener() {
        ...
        RxView.clicks(mTvRechargeRule)
                ...
                .subscribe {
                    ...
                    val bundle = Bundle()
                    bundle.putString(WalletRuleFragment.BUNDLE_RULE, mSystemConfigBean.currency.cash.rule)
        ...
        RxView.clicks(mBtSure)
                ...
                .subscribe {
                    when {
                        mRechargeMoney <  mSystemConfigBean.currency.settings.cashmin -> showSnackErrorMessage(getString(R.string.please_more_than_min_withdrawals_formart,
                                mSystemConfigBean.currency.settings.cashmin))
                        mRechargeMoney > mSystemConfigBean.currency.settings.cashmax -> showSnackErrorMessage(getString(R.string.please_less_max_withdrawals_formart, mSystemConfigBean.currency.settings.cashmax))
