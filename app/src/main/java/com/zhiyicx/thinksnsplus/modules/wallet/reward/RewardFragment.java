...
public class RewardFragment extends TSFragment<RewardContract.Presenter> implements RewardContract.View {
    ...
    protected void setRightClick() {
        super.setRightClick();
        if (mIlvPassword.getVisibility() == View.GONE) {
    ...
    public void onSureClick(View v, String text, InputPasswordView.PayNote payNote) {
        mPresenter.reward(mRewardMoney, mRewardType, mSourceId, payNote.psd);
    }
                ...
                .subscribe(aVoid -> {
                    if (!mEtInput.getText().toString().isEmpty() && mRewardMoney != (int) mRewardMoney) {
                        ...
                        if (mPresenter.usePayPassword()) {
                            ...
                            mPresenter.reward(mRewardMoney, mRewardType, mSourceId, null);
                        }
                    }
                });// 传入的是真实货币分单位
