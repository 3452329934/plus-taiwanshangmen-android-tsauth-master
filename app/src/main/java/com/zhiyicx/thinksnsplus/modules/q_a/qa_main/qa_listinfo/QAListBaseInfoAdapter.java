...
public class QAListBaseInfoAdapter<T extends BaseListBean> extends CommonAdapter<T> {
    ...
    protected void convert(ViewHolder holder, T data, int position) {
        ...
        if (isNeedShowAnswer()) {
            ...
            UserAvatarView userAvatarView = contentView.findViewById(R.id.iv_head_icon);
            TextView contentTextView = contentView.findViewById(R.id.item_info_hotcomment);
            ...
            if (infoBean.getAnswer() != null) {
                ...
                boolean isAnonymity = infoBean.getAnswer().getAnonymity() == 1 || infoBean.getAnswer().getUser() == null;

                // 正文
                String content = infoBean.getAnswer().getText_body();

                if (TextUtils.isEmpty(content)) {
                    content = infoBean.getAnswer().getBody();
                }
                ...
                String prefix = (isAnonymity && !isSelf ? getContext().getString(R.string.qa_question_answer_anonymity_user)
                        : infoBean.getAnswer().getUser().getName()) + (isSelf && isAnonymity ? mContext.getString(R.string
                        .qa_question_answer_anonymity_current_user) : "") + "：";
                if (!canLook) {
...
//                          直接进入问题详情
                            ...
                            Bundle bundle = new Bundle();
                            bundle.putSerializable(BUNDLE_QUESTION_BEAN, infoBean);
