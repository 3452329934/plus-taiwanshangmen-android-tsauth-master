...
public class MessageCommentPresenter extends AppBasePresenter<MessageCommentContract.View> implements
    ...
    public void requestNetData(Long maxId, final boolean isLoadMore) {
        Subscription commentSub = mNotificationRepository.makeNotificationReaded(INotificationRepository.COMMENT)
                .observeOn(Schedulers.io())
                .flatMap((Func1<Object, Observable<List<CommentedBean>>>) o -> mBaseDynamicRepository.getCommentMessages(INotificationRepository.COMMENT,
                        maxId.intValue()))
    ...
    public void sendComment(int mCurrentPostion, long replyToUserId, String commentContent) {
        final CommentedBean currentCommentBean = mRootView.getListDatas().get(mCurrentPostion);
        String path = CommentRepository.getCommentPath(currentCommentBean.getTarget_id(),
                currentCommentBean.getChannel(), currentCommentBean.getSource_id());
                ...
                .subscribe(new BaseSubscribeForV2<BaseJsonV2<Object>>() {
                    ...
                    protected void onException(Throwable throwable) {
                        ...
                        if (throwable instanceof HttpException) {
                            if (((HttpException) throwable).code() == ErrorCodeConfig.DATA_HAS_BE_DELETED) {
                                currentCommentBean.setCommentable(null);
...
//        }
        ...
        Subscription subscribe = handleIntegrationBlance(amount)
                ...
                .flatMap(o -> mCommentRepository.paykNote(note, null))
