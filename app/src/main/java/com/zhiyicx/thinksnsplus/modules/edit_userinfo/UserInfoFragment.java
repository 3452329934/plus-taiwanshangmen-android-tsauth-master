...
//                intent.putExtras(bundle);
    ...
    private UpdateUserInfoTaskParams packageUserInfo() {
        ...
        // 只上传改变的信息
        if (userNameChanged) {
            userInfoTaskParams.setName(mEtUserName.getText().toString());
        }
        if (sexChanged) {
            userInfoTaskParams.setSex((Integer) mTvSex.getTag(R.id.view_data));
        }
        if (cityChanged) {
            userInfoTaskParams.setLocation(mCurrentShowLocation);
        }
        if (introduceChanged) {
            userInfoTaskParams.setBio(mEtUserIntroduce.getInputContent());
        }

        return userInfoTaskParams;
    }
