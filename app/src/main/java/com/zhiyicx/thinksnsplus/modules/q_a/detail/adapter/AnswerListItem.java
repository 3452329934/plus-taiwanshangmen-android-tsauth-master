...
public class AnswerListItem implements ItemViewDelegate<AnswerInfoBean> {
    ...
    public AnswerListItem(@NotNull QuestionDetailContract.Presenter mPresenter, Context context) {
    ...
    public void convert(ViewHolder holder, AnswerInfoBean answerInfoBean, AnswerInfoBean lastT,
        ...
        boolean isOnlook = mPresenter.getCurrentQuestion().getLook() == 1; // 是否开启了围观
        ...
        boolean isAnonymity = answerInfoBean.getAnonymity() == 1 || answerInfoBean.getUser() == null;
        ...
        ImageUtils.loadCircleUserHeadPic(answerInfoBean.getUser(), holder.getView(R.id
                .iv_portrait), !isMineAnwser && isAnonymity);
        TextView nameView = holder.getTextView(R.id.tv_name);
        if (isAnonymity) {
            nameView.setText(!isMineAnwser ? nameView.getResources().getString(R.string
        ...
        // 邀请的人回答才会有围观
        tvToWatch.setVisibility(isOnlook && isInvited && !isMineAnwser ? View.VISIBLE : View.GONE);
        // 是否已经围观了
        tvToWatch.setEnabled(!answerInfoBean.getCould());
        tvToWatch.setCompoundDrawables(null, null, null, null);
        ...
        // 没有开启围观是自己发布的，并且没有被采纳,设置采纳按钮
        boolean isMineQuestion = mPresenter.getCurrentQuestion().getUser_id() == AppApplication.getMyUserIdWithdefault();
        boolean hasNoAdoptionAnswers = mPresenter.getCurrentQuestion().getAdoption_answers() == null || mPresenter.getCurrentQuestion()
                .getAdoption_answers().isEmpty();
        ...
        // 不是自己的回答，不是自己的问题，没有采纳的且邀请的页没有被采纳
        if (!isMineAnwser && isMineQuestion && hasNoAdoptionAnswers && !hasAdoptionInviteAnswer) {
            tvToWatch.setVisibility(View.VISIBLE);
            tvToWatch.setEnabled(true);
            tvToWatch.setTextColor(ContextCompat.getColor(tvToWatch
                    .getContext(), R.color.themeColor));
            tvToWatch.setText(R.string.qa_question_answer_adop);
            tvToWatch.setBackgroundResource(R.drawable.selector_button_corner_hollow_small);
            tvToWatch.setCompoundDrawables(UIUtils.getCompoundDrawables(tvToWatch.getContext(), R.mipmap.detail_ico_adopt_high), null, null, null);
    ...
    protected List<Link> setLiknks(final AnswerInfoBean answerInfoBean, String content) {
        ...
        if (content.contains(Link.DEFAULT_NET_SITE)) {
            Link commentNameLink = new Link(Link.DEFAULT_NET_SITE)
                    ...
                    .setOnClickListener((clickedText, linkMetadata) -> CustomWEBActivity.startToWEBActivity(mContext, clickedText))
