...
public class WalletFragment extends TSFragment<WalletContract.Presenter> implements WalletContract.View {
    ...
    protected void initView(View rootView) {
        ...
        if (mSystemConfigBean == null ||  !mSystemConfigBean.getWallet().getRecharge().isStatus()) {
            mBtReCharge.setVisibility(View.GONE);
        } else {
            mBtReCharge.setVisibility(View.VISIBLE);
        }
        if (mSystemConfigBean == null || mSystemConfigBean.getWallet().getCash() == null || !mSystemConfigBean.getWallet().getCash().isStatus()) {
...
//                .subscribe(aVoid -> mPresenter.checkWalletConfig(WalletPresenter.TAG_RECHARGE, true));
    ...
    public void updateBalance(double balance) {
        mTvMineMoney.setText(getString(R.string.money_format, PayConfig.realCurrencyFen2Yuan(balance)));
    }
     ...
     */
    @Override
    public void walletConfigCallBack(SystemConfigBean.WalletConfigBean walletConfigBean, int tag) {
        Bundle bundle = new Bundle();
        switch (tag) {
