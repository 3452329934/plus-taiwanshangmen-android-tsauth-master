...
public class MessageFragment extends TSListFragment<MessageContract.Presenter, MessageItemBean>
    ...
    private void updateHeaderViewData(View headerview, MessageItemBean systemMsgItemData, MessageItemBean commentItemData,
                                      MessageItemBean likedItemData, MessageItemBean reviewItemBean, MessageItemBean atItemData) {
        ...
        TextView tvHeaderAtContent = null;
        TextView tvHeaderAtTime = null;
        BadgeView tvHeaderAtTip = null;
        ...
        RxView.clicks(headerview.findViewById(R.id.rl_at_message))
                .throttleFirst(JITTER_SPACING_TIME, TimeUnit.SECONDS)
                .subscribe(aVoid -> {
                    if (((MessageAdapterV2) mAdapter).hasItemOpend()) {
                        ((MessageAdapterV2) mAdapter).closeAllItems();
                    ...
                    // 跳转系统通知页面
                    toAtPage();
                    mPresenter.updateAtMsgItemData().setUnReadMessageNums(0);
                    updateCommnetItemData(mPresenter.updateAtMsgItemData());
                });
        ...
        tvHeaderSystemMsgContent = headerview.findViewById(R.id
                .tv_header_notification_content);
        tvHeaderSystemMsgTime = headerview.findViewById(R.id.tv_header_notification_time);
        tvHeaderSystemMsgTip = headerview.findViewById(R.id.tv_header_notification_tip);

        tvHeaderCommentContent = headerview.findViewById(R.id
                .tv_header_comment_content);
        tvHeaderCommentTime = headerview.findViewById(R.id.tv_header_comment_time);
        tvHeaderCommentTip = headerview.findViewById(R.id.tv_header_comment_tip);

        tvHeaderLikeContent = headerview.findViewById(R.id.tv_header_like_content);
        tvHeaderLikeTime = headerview.findViewById(R.id.tv_header_like_time);
        tvHeaderLikeTip = headerview.findViewById(R.id.tv_header_like_tip);

        tvHeaderReviewContent = headerview.findViewById(R.id
                .tv_header_review_content);
        tvHeaderReviewTime = headerview.findViewById(R.id.tv_header_review_time);
        tvHeaderReviewTip = headerview.findViewById(R.id.tv_header_review_tip);

        tvHeaderAtContent = headerview.findViewById(R.id
                .tv_header_at_content);
        tvHeaderAtTime = headerview.findViewById(R.id.tv_header_at_time);
        tvHeaderAtTip = headerview.findViewById(R.id.tv_header_at_tip);
        ...
        if (systemMsgItemData.getConversation().getLast_message_time() == 0 || (systemMsgItemData
                .getConversation().getLast_message().getTxt() != null && systemMsgItemData
        ...
        tvHeaderCommentContent.setText(DeviceUtils.getSubStringIndex(tvHeaderCommentContent.getPaint(), commentText, maxW, mActivity.getString(threePeople ? R.string.comment_me_more : R.string.comment_me)));

        if (commentItemData.getConversation().getLast_message_time() == 0 || (commentItemData
                .getConversation().getLast_message().getTxt() != null && commentItemData
                .getConversation().getLast_message().getTxt().contains(getString(R.string
                        .has_no_body)))) {
        ...
        String atText = atItemData.getConversation().getLast_message().getTxt();
        threePeople = atText.endsWith(mActivity.getString(R.string.at_me_more));
        tvHeaderAtContent.setText(DeviceUtils.getSubStringIndex(tvHeaderAtContent.getPaint(), atText, maxW, mActivity.getString(threePeople ? R.string.at_me_more : R.string.at_me)));

        if (atItemData.getConversation().getLast_message_time() == 0 || (atItemData
                .getConversation().getLast_message().getTxt() != null && atItemData
                .getConversation().getLast_message().getTxt().contains(getString(R.string
                        .has_no_body)))) {
            tvHeaderAtTime.setVisibility(View.INVISIBLE);
        } else {
            tvHeaderAtTime.setVisibility(View.VISIBLE);
            tvHeaderAtTime.setText(TimeUtils.getTimeFriendlyNormal(atItemData
                    .getConversation().getLast_message_time()));
        }
        tvHeaderAtTip.setBadgeCount(Integer.parseInt(ConvertUtils.messageNumberConvert
                (atItemData.getUnReadMessageNums())));
        ...
        tvHeaderLikeContent.setText(DeviceUtils.getSubStringIndex(tvHeaderLikeContent.getPaint(), likeText, maxW, mActivity.getString(threePeople ? R.string.like_me_more : R.string.like_me)));

        if (likedItemData.getConversation().getLast_message_time() == 0 || likedItemData
                .getConversation().getLast_message().getTxt() != null && likedItemData
                .getConversation().getLast_message().getTxt().contains(getString(R.string
                        .has_no_body))) {
        ...
        if (reviewItemBean.getConversation().getLast_message_time() == 0 || (reviewItemBean
                .getConversation().getLast_message().getTxt() != null && reviewItemBean
                .getConversation().getLast_message().getTxt().contains(getString(R.string
                        .has_no_body)))) {
    ...
    private void toAtPage() {
        startActivity(new Intent(getContext(), MessageAtActivity.class));
    }
    ...
    private void toReviewList() {
        ...
        bundle.putParcelable(MessageReviewFragment.BUNDLE_PINNED_DATA, mPresenter.getUserFollowerCountBean());
        to.putExtras(bundle);
        startActivity(to);
    }
    ...
    public void updateSystemMsgItemData(MessageItemBean messageItemBean) {
        ...
        updateHeaderViewData(mHeaderView, mPresenter.updateSystemMsgItemData(), mPresenter.updateCommnetItemData(), mPresenter
                .updateLikeItemData(), mPresenter.updateReviewItemData(), mPresenter.updateAtMsgItemData());
        refreshData();
    }
