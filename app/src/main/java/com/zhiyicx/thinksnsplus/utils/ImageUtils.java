...
//                .signature(new StringSignature(String.valueOf(mCoverSigture)))
    ...
    public static void loadCircleUserHeadPicWithBorder(UserInfoBean userInfoBean, UserAvatarView imageView, int width, int color) {
        loadUserHead(userInfoBean, imageView, true, width, color);
    }
...
//                    .signature(new StringSignature(String.valueOf(mHeadPicSigture)))
    ...
    public static void loadUserHead(UserInfoBean userInfoBean, UserAvatarView imageView, boolean withBorder, int width, int color) {
        if (checkImageContext(imageView)) {
            return;
        }

        loadUserAvatar(userInfoBean, imageView.getIvAvatar(), withBorder, width, color);
        if (userInfoBean != null && userInfoBean.getVerified() != null && !TextUtils.isEmpty(userInfoBean.getVerified().getType())) {
            if (TextUtils.isEmpty(userInfoBean.getVerified().getIcon())) {
                userInfoBean.getVerified().setIcon("");
            }
