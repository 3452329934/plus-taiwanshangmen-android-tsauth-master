...
### 第三方账号配置
 ...
 项目使用中的三方账号包括：友盟、环信、新浪微博、微信、QQ、极光推送、高德地图、支付宝支付。请在相应平台申请后修改配置文件,三方申请注册需要的资料请查看[移动端打包上线需要得资料说明](AppPackageInfoTutorial.md)。
...
4. QQ、Sina、Wixin 三方账号 key 配置。位于 `baseproject/src/main/java/com.zhiyicx.baseproject/config/UmengConfig.class`。 QQ 的 `QQ_APPID` 也需要在`baseproject/build.gradle`中配置
    ...
    ![share_packge_change]
5. 环信配置，修改高德 `EASEMOB_APPKEY`，位于 `baseproject/build.gradle`
    ```
        apply plugin: 'com.android.library'
        android {
        ...
                // 环信
                     manifestPlaceholders = [
                             EASEMOB_APPKEY: "1139171207178196#thinksns-plus",
                     ]
